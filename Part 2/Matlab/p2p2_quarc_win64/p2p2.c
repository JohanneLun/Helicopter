 ...
 * C source code generated on : Sat Oct 07 12:01:30 2017
 *
...
void p2p2_output0(void)                /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(p2p2_M)) {
    ...
    if (p2p2_B.RateTransitionx > p2p2_P.DeadZonex_End) {
      rtb_Sum_p = p2p2_B.RateTransitionx - p2p2_P.DeadZonex_End;
    } else if (p2p2_B.RateTransitionx >= p2p2_P.DeadZonex_Start) {
      rtb_Sum_p = 0.0;
    } else {
      rtb_Sum_p = p2p2_B.RateTransitionx - p2p2_P.DeadZonex_Start;
    }
     ...
     *  Gain: '<S4>/Gain: x'
     */
    p2p2_B.Joystick_gain_x = p2p2_P.Gainx_Gain * rtb_Sum_p *
      p2p2_P.Joystick_gain_x;

    /* Gain: '<Root>/Gain' */
    p2p2_B.Gain = p2p2_P.Gain_Gain * p2p2_B.Joystick_gain_x;
  ...
  /* Gain: '<S13>/Gain' */
  p2p2_B.Gain_d = p2p2_P.Gain_Gain_l * rtb_Backgain;
  if (rtmIsMajorTimeStep(p2p2_M)) {
    ...
    /* Gain: '<S10>/Gain' */
    p2p2_B.Gain_i = p2p2_P.Gain_Gain_ar * p2p2_B.PitchCounttorad;
  }
  ...
  /* Gain: '<S11>/Gain' */
  p2p2_B.Gain_b = p2p2_P.Gain_Gain_ae * rtb_Backgain;
  if (rtmIsMajorTimeStep(p2p2_M)) {
     ...
     *  Gain: '<S8>/Gain'
     */
    p2p2_B.Sum = p2p2_P.Gain_Gain_lv * p2p2_B.ElevationCounttorad -
  ...
  /* Gain: '<S9>/Gain' */
  p2p2_B.Gain_dg = p2p2_P.Gain_Gain_n * rtb_Backgain;

  /* Gain: '<Root>/deg to rad' */
  p2p2_B.degtorad[0] = p2p2_P.degtorad_Gain * p2p2_B.Gain_p;
  p2p2_B.degtorad[1] = p2p2_P.degtorad_Gain * p2p2_B.Gain_d;
  p2p2_B.degtorad[2] = p2p2_P.degtorad_Gain * p2p2_B.Gain_i;
  p2p2_B.degtorad[3] = p2p2_P.degtorad_Gain * p2p2_B.Gain_b;
  p2p2_B.degtorad[4] = p2p2_P.degtorad_Gain * p2p2_B.Sum;
  p2p2_B.degtorad[5] = p2p2_P.degtorad_Gain * p2p2_B.Gain_dg;
  if (rtmIsMajorTimeStep(p2p2_M)) {
    /* SignalConversion: '<Root>/TmpSignal ConversionAtTo FileInport1' */
    rtb_TmpSignalConversionAtToFile[0] = p2p2_B.Gain;
    rtb_TmpSignalConversionAtToFile[1] = p2p2_B.degtorad[1];
    rtb_TmpSignalConversionAtToFile[2] = p2p2_B.degtorad[2];

    /* ToFile: '<Root>/To File' */
    if (rtmIsMajorTimeStep(p2p2_M)) {
      {
        if (!(++p2p2_DW.ToFile_IWORK.Decimation % 1) &&
            (p2p2_DW.ToFile_IWORK.Count*4)+1 < 100000000 ) {
          ...
          if (fp != (NULL)) {
            ...
            if (fwrite(u, sizeof(real_T), 4, fp) != 4) {
              rtmSetErrorStatus(p2p2_M, "Error writing to MAT-file data.mat");
              return;
            }

            if (((++p2p2_DW.ToFile_IWORK.Count)*4)+1 >= 100000000) {
              (void)fprintf(stdout,
  ...
  if (p2p2_M->Timing.t[0] < p2p2_P.t_d) {
    rtb_Backgain = p2p2_P.waitt_dsec_Y0;
  } else {
    rtb_Backgain = p2p2_P.waitt_dsec_YFinal;
  }
   ...
   *  Sum: '<S6>/Sum'
   */
  p2p2_B.Product = (((p2p2_B.Gain - p2p2_B.degtorad[1]) * p2p2_P.K_rp -
                     p2p2_B.degtorad[2]) * p2p2_P.K_pp - p2p2_P.K_pd *
                    p2p2_B.degtorad[3]) * rtb_Backgain;
  if (rtmIsMajorTimeStep(p2p2_M)) {
  }
  ...
  if (rtmIsMajorTimeStep(p2p2_M)) {
    /* Constant: '<Root>/e ref' */
    p2p2_B.eref = p2p2_P.eref_Value;
  }

  /* Sum: '<S2>/Sum' */
  rtb_Sum_p = p2p2_B.eref - p2p2_B.degtorad[4];

  /* Sum: '<S7>/Sum' incorporates:
   *  Gain: '<S7>/K_ed'
   *  Gain: '<S7>/K_ep'
   */
  p2p2_B.Sum_l = (p2p2_P.K_ep_Gain * rtb_Sum_p + rtb_Backgain) -
   ...
   *  Sum: '<S1>/Subtract'
   */
  rtb_Backgain = (p2p2_B.Sum_l - p2p2_B.Product) * p2p2_P.Backgain_Gain;
   ...
   *  Sum: '<S1>/Add'
   */
  rtb_Sum_p = (p2p2_B.Product + p2p2_B.Sum_l) * p2p2_P.Frontgain_Gain;

  /* Saturate: '<S3>/Front motor: Saturation' */
  if (rtb_Sum_p > p2p2_P.FrontmotorSaturation_UpperSat) {
    p2p2_B.FrontmotorSaturation = p2p2_P.FrontmotorSaturation_UpperSat;
  } else if (rtb_Sum_p < p2p2_P.FrontmotorSaturation_LowerSat) {
    p2p2_B.FrontmotorSaturation = p2p2_P.FrontmotorSaturation_LowerSat;
  } else {
    p2p2_B.FrontmotorSaturation = rtb_Sum_p;
  }
  ...
  if (rtmIsMajorTimeStep(p2p2_M)) {
    ...
    if (p2p2_B.RateTransitiony > p2p2_P.DeadZoney_End) {
      rtb_Sum_p = p2p2_B.RateTransitiony - p2p2_P.DeadZoney_End;
    } else if (p2p2_B.RateTransitiony >= p2p2_P.DeadZoney_Start) {
      rtb_Sum_p = 0.0;
    } else {
      rtb_Sum_p = p2p2_B.RateTransitiony - p2p2_P.DeadZoney_Start;
    }
     ...
     *  Gain: '<S4>/Gain: y'
     */
    p2p2_B.Joystick_gain_y = p2p2_P.Gainy_Gain * rtb_Sum_p *
...
void p2p2_initialize(void)
{
  ...
  /* Start for ToFile: '<Root>/To File' */
  {
    ...
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(p2p2_M, "Error creating .mat file data.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp,4,0,"data")) {
      rtmSetErrorStatus(p2p2_M, "Error writing mat file header to file data.mat");
      return;
    }
...
void p2p2_terminate(void)
{
  ...
  /* Terminate for ToFile: '<Root>/To File' */
  {
    ...
    if (fp != (NULL)) {
      char fileName[509] = "data.mat";
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(p2p2_M, "Error closing MAT-file data.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(p2p2_M, "Error reopening MAT-file data.mat");
        return;
      }

      if (rt_WriteMat4FileHeader(fp, 4, p2p2_DW.ToFile_IWORK.Count, "data")) {
...
RT_MODEL_p2p2_T *p2p2(void)
{
  ...
  /* External mode info */
  p2p2_M->Sizes.checksums[0] = (3778692049U);
  p2p2_M->Sizes.checksums[1] = (228347801U);
  p2p2_M->Sizes.checksums[2] = (2250247387U);
  p2p2_M->Sizes.checksums[3] = (4101635893U);
